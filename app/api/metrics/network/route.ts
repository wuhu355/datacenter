  ...
  try {
    ...
    const data = await getCached<MetricPoint[]>(cacheKey, async () => {
      ...
      const [maxRows] = await pool.execute(
        `SELECT MAX(ts) AS maxTs FROM pref_tsar WHERE tag = 'net_speed_mb'`
      ) as any
      ...
      const [rows] = await pool.execute(
        `SELECT p.ts, p.mod, p.value
         FROM pref_tsar p
         WHERE p.tag = 'net_speed_mb' AND p.ts >= ? ${hostFilter}
         ORDER BY p.ts ASC`,
        params
      ) as any

      // Pivot dynamically: mod values (net_in, net_out) become columns
